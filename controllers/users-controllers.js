...
const getUsers = async (req, res, next) => {
    let users;
    try {
        users = await User.find({}, '-password'); // return witout password
    } catch (err) {
        const error = new HttpError('Fetching users falied, please try again later.');
        return next(error);
    }
...
const login = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError('Could not log you in, please check your credential and try again.', 500);
        return next(error);
    }
